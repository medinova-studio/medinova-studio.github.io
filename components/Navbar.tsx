    ...
    >
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Brand */}
          <a href="#" className="flex items-center gap-2.5 group" aria-label="Medinova Studio">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="nav-link relative text-sm text-slate-400 hover:text-cyan transition-colors duration-200"
              >
                {l.label}
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan/10 border border-cyan/20 text-cyan text-sm font-medium hover:bg-cyan/20 hover:border-cyan/40 transition-all duration-200"
            >
              Get Started
            </a>
          </div>
